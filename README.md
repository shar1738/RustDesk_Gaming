...
THE ISSUE 
Everything runs well except for getting your mouse out of the radius when in rust desk. Since rust desk captures your keyboard in its software, it voids all the autohotkeys in the program. This is bad because now you just have a trapped mouse in the middle of the screen with no where to go. THE FIX FOR NOW other keys work on the keyboard to access your current pc again find the ones that work for you, if your using windows hit ctrl-alt-delete in order to regain control then toggle off the program and then hit cancel so you don't shut down or log out your pc. This is the only solution I could find to this issue I could find without modifying the rust desk code which is a nightmare. Please if you have a solution, please share, but for now, just do this (I know its not ideal but it is what it is) 
